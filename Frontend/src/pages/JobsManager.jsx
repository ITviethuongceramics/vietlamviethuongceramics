  ...
  return (
    <>
      <div className="adm-topbar" style={{ marginBottom: 20 }}>
        <div></div>
        <button className="adm-btn adm-btn--primary" onClick={() => { setShowForm(true); setEditJob(null); setForm(emptyForm); setLocationMode('select'); }}>
      ...
      {showForm && (
        <div className="adm-form-card">
          <div className="adm-form-card__header">
            ...
            <button className="adm-icon-btn" onClick={() => { setShowForm(false); setEditJob(null); }}><X size={18} /></button>
          </div>
          <form onSubmit={handleSubmit} className="adm-form">
            <div className="adm-form__grid">
              {/* Các field giữ nguyên như cũ */}
              ...
              <div className="adm-field">
                ...
                <div className="adm-select-wrap">
                  <select value={['Kinh doanh', 'Marketing', 'Kế toán - Tài chính', 'Nhân sự', 'Kỹ thuật - Sản xuất', 'Logistics', 'IT', 'Hành chính', 'Ban Giám Đốc'].includes(form.department) ? form.department : (form.department ? 'other' : '')}
                    onChange={e => {
                      if (e.target.value === 'other') setForm({ ...form, department: '' });
                ...
                {!['Kinh doanh', 'Marketing', 'Kế toán - Tài chính', 'Nhân sự', 'Kỹ thuật - Sản xuất', 'Logistics', 'IT', 'Hành chính', 'Ban Giám Đốc', ''].includes(form.department) && (
                  <input style={{ marginTop: 8 }} value={form.department} onChange={e => setForm({ ...form, department: e.target.value })} placeholder="Nhập tên phòng ban..." autoFocus />
                )}
              </div>
              <div className="adm-field">
                <label>Địa điểm</label>
                <div className="adm-select-wrap">
                  <select value={locationMode === 'custom' ? 'other' : form.location} onChange={e => {
                    if (e.target.value === 'other') { setLocationMode('custom'); setForm({ ...form, location: '' }); }
                    else { setLocationMode('select'); setForm({ ...form, location: e.target.value }); }
                  }}>
                ...
                {locationMode === 'custom' && (
                  <input style={{ marginTop: 8 }} value={form.location} onChange={e => setForm({ ...form, location: e.target.value })} placeholder="Nhập địa điểm..." autoFocus />
                )}
              </div>
              <div className="adm-field">
                ...
                <div className="adm-select-wrap">
                  <select value={form.type} onChange={e => setForm({ ...form, type: e.target.value })}>
                    ...
                    <option>Toàn thời gian</option><option>Bán thời gian</option><option>Thực tập</option>
                  </select>
                  <ChevronDown size={14} />
                </div>
              </div>
              ...
              <div className="adm-field">
                ...
                <div className="adm-select-wrap">
                  <select value={form.status} onChange={e => setForm({ ...form, status: e.target.value })}>
                    <option value="active">Đang tuyển</option><option value="inactive">Ngừng tuyển</option>
                  </select>
                  <ChevronDown size={14} />
                </div>
              </div>
            ...
            <div className="adm-form__actions">
              <button type="button" className="adm-btn adm-btn--ghost" onClick={() => { setShowForm(false); setEditJob(null); }}><X size={15} /> Hủy</button>
              <button type="submit" className="adm-btn adm-btn--primary"><Save size={15} /> {editJob ? 'Cập nhật' : 'Đăng tin'}</button>
            </div>
          </form>
        </div>
      )}
        ...
        <>
          <div className="adm-table-wrap">
            <table className="adm-table">
              <thead><tr><th>Vị trí</th><th>Địa điểm</th><th>Loại</th><th>Hạn nộp</th><th>Trạng thái</th><th>Thao tác</th></tr></thead>
              <tbody>
                {pagedJobs.length === 0 ? (
                    ...
                    <tr key={job.id}>
                      ...
                      <td>
                        <div className="adm-table__actions">
                          <button className="adm-icon-btn adm-icon-btn--blue" onClick={() => handleEdit(job)} title="Sửa"><Pencil size={14} /></button>
                          <button className="adm-icon-btn adm-icon-btn--red" onClick={() => handleDelete(job.id)} title="Xóa"><Trash2 size={14} /></button>
                        </div>
                      </td>
                    </tr>
                  );
          ...
          <div className="adm-table-footer">
            ...
            <Pagination page={jobPage} total={jobTotalPages} onChange={p => { setJobPage(p); window.scrollTo({ top: 0, behavior: 'smooth' }); }} />
          </div>
        </>
      )}
    </>
